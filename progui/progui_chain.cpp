...
static inline bool IsTurnToken(const std::string& s) {
 return (s == "left" || s == "right" || s == "up" || s == "down" || s == "Turnfront" || s == "TurnBack");
}
...
void spawnCube()
{
 ...
 if (g_lastBody && !g_suppressDirRecord) {
 RecordForwardInput();
 }
...
bool SaveChainToFile(const char *filename)
{
 ...
 if (!ofs.is_open())
 {
 std::cerr << "SaveChainToFile: could not open file: " << out << "\n";
 return false;
 }
 for (const auto &dir : g_directionHistory)
 {
 ...
 std::cout << "Saved direction history to: " << out << " (" << g_directionHistory.size() << " tokens)\n";
 return true;
}
...
bool LoadChainFromFile(const char *filename)
{
 ...
 if (!ifs.is_open())
 {
 std::cerr << "LoadChainFromFile: could not open: " << filename << "\n";
 return false;
 }
 ...
 // Clear current chain to just the root body; rebuild fresh
 if (!spec || !m || !d)
 {
 std::cerr << "LoadChainFromFile: model/spec not ready\n";
 return false;
 }

 // Delete all cubes except the first one
 while (g_chain.size() >1)
 {
 deleteLastCube();
 }

 // Ensure we have a root cube; if empty, spawn one without recording
 if (g_chain.empty())
 {
 g_suppressDirRecord = true;
 spawnCube();
 g_suppressDirRecord = false;
 }

 // Reset spawn face to +X default to have deterministic replay
 g_spawnFace = FACE_POSX;
 UpdateMarkerOnLastBody();

 // Replay tokens: non-forward adjust face, forward spawns
 g_suppressDirRecord = true;
 for (size_t ti =0; ti < tokens.size(); ++ti)
 {
 const auto &t = tokens[ti];
 spawnCubeFromInput(t.c_str());

 // Render incremental progress so user sees the chain building
 GLFWwindow* wnd = glfwGetCurrentContext();
 if (wnd && m && d) {
 mjrRect viewport = {0,0,0,0};
 glfwGetFramebufferSize(wnd, &viewport.width, &viewport.height);
 // update scene from current state and render
 mjv_updateScene(m, d, &opt, NULL, &cam, mjCAT_ALL, &scn);
 // draw scene
 mjr_render(viewport, &scn, &con);
 // draw a small overlay with progress
 char progress[128];
 std::snprintf(progress, sizeof(progress), "Loading: %zu / %zu", ti+1, tokens.size());
 mjr_overlay(mjFONT_NORMAL, mjGRID_TOPLEFT, viewport, progress, NULL, &con);
 // swap and poll so UI remains responsive
 glfwSwapBuffers(wnd);
 glfwPollEvents();
 }
 }
 ...
 // Replace direction history with the loaded tokens
 g_directionHistory = tokens;

 // Recompile/render
 int old_nv = m->nv, old_na = m->na;
 if (mj_recompile(spec, nullptr, m, d) !=0)
 {
 std::cerr << "LoadChainFromFile: recompile failed: " << mjs_getError(spec) << "\n";
 return false;
 }
 ...
 std::cout << "Loaded chain from directions file: " << filename << " (" << tokens.size() << " tokens)\n";
 return true;
}

void spawnCubeFromInput(const char* dirToken)
{
 if (!dirToken)
 {
 spawnCube();
 ...
 // Determine the new face based on current face and token
 int newFace = g_spawnFace;
 const std::string tok = std::string(dirToken);

 auto turnRight = [](int face)->int{
     switch (face)
     {
     case FACE_POSX:
         return FACE_NEGY;
     ...
     case FACE_NEGY:
         return FACE_NEGX;
     case FACE_POSZ:
         return FACE_POSZ; // right/left in XY plane only; keep Z facing
     case FACE_NEGZ:
         return FACE_NEGZ;
     default:
         return face;
     ...
     auto faceUp = [](int /*face*/)->int{ return FACE_POSZ; };
     auto faceDown = [](int /*face*/)->int{ return FACE_NEGZ; };
     ...
     {
     newFace = turnRight(g_spawnFace);
     }
     else if (tok == "left")
     {
     newFace = turnLeft(g_spawnFace);
     }
     else if (tok == "up")
     {
     newFace = faceUp(g_spawnFace);
     }
     else if (tok == "down")
     {
     newFace = faceDown(g_spawnFace);
     }
     else if (tok == "Turnfront")
     {
     // interpret as turning from Z to XY towards the positive new face
     // choose a deterministic mapping: when currently Z, map to +X
     if (g_spawnFace == FACE_POSZ || g_spawnFace == FACE_NEGZ)
     newFace = FACE_POSX;
     }
     else if (tok == "TurnBack")
     {
     // interpret as turning from Z to XY towards the negative new face
     if (g_spawnFace == FACE_POSZ || g_spawnFace == FACE_NEGZ)
     newFace = FACE_NEGX;
     }
     else
 ...
 // Now spawn a cube using current face
 spawnCube();
}
